# Fusion Profile Packs

Community-made profile pictures for Fusion. Create a pack, share one link, and anyone can use your pictures on their profiles, on iPhone, iPad, Mac and Apple TV.

## Add a pack to Fusion

1. Copy a pack link (a URL to a `.json` file — see [Packs](#packs) below or make your own).
2. In Fusion, go to **Settings → Profiles → Profile Pictures**, paste the link, and tap **Add**.

## Use an picture on a profile
...
3. Choose **Change Picture** and pick an image from any of your packs. **Remove Picture** switches back to the default icon.

Packs are shared by all profiles: add a pack once and every profile can pick from it. The pack list also syncs to your other devices via iCloud, and each profile's chosen picture travels with the profile. Packs auto-refresh once a day, so when the creator adds new images you get them automatically.
...
```json
{
  "title": "My Awesome profile pictures",
  "images": [
...
- `name` — the label under each picture.
...
- **Keep image URLs stable.** Users who picked a picture keep pointing at the URL they chose. If you must move files, keep the same `name` in the JSON — Fusion refreshes the pack daily and users re-picking will get the new URL.
...
**What happens if a pack goes offline?** Pictures already in use keep displaying from cache; the pack just stops refreshing. Worst case a profile falls back to its default icon. Nothing breaks.

**Does removing a pack remove my picture?** Your chosen picture keeps its image (the URL is stored with the profile). Removing a pack only takes it out of the picker — for every profile, since packs are shared.
